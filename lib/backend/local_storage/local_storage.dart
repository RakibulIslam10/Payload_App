...
const int isSmsVerifyKey = 0;

const String isSmsKey = "isSmsKey";
...
const String privacyPolicyLink = "privacy Policy Link";
const String aboutUsLink = "About Us Link";
const String contactUsLink = "contact Us Link";

class LocalStorage {
  static Future<void> savePrivacyPolicyLink({required String link}) async {
    final box = GetStorage();

    await box.write(privacyPolicyLink, link);
    debugPrint(link);
  }

  static String getPrivacyPolicyLink() {
    return GetStorage().read(privacyPolicyLink) ?? '';
  }

  static Future<void> saveAboutUsLink({required String link}) async {
    final box = GetStorage();

    await box.write(aboutUsLink, link);
    debugPrint(link);
  }

  static String getAboutUsLink() {
    return GetStorage().read(aboutUsLink) ?? '';
  }

  static Future<void> saveContactUsLink({required String link}) async {
    final box = GetStorage();

    await box.write(contactUsLink, link);
    debugPrint(link);
  }

  static String getContactUsLink() {
    return GetStorage().read(contactUsLink) ?? '';
  }
  ...
  static Future<void> saveSmsVerify({required int id}) async {
    final box = GetStorage();
